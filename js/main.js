import {
  PERSON, ROLE_PROFILES, DEFAULT_ROLE, CAREER_PROGRESSION,
  ...
  RESUME_MODES, COMPANY_PROFILES, DEFAULT_COMPANY, EVIDENCE_ITEMS,
  WHY_I_BUILT_THIS, WHY_ANNOTATIONS,
} from "./data.js";
...
const currentCompany = COMPANY_PROFILES[urlCompany] ? urlCompany : DEFAULT_COMPANY;
...
function setCompanyInUrl(company) {
  const url = new URL(location.href);
...
/* ============================================================
   ROLE TARGETING — no on-page picker anymore (removed: it added
   a decision the visitor had to make before the JTBD — "understand
   Joe in 3 minutes" — was served). The underlying reorder-by-role
   machinery still works, driven purely by ?role=<slug> in the URL,
   so Joe can still hand-craft a tailored link per application.
   ============================================================ */

/* ============================================================
   RENDER: EXPERIENCE — career progression arc + intro
   ============================================================ */
function renderExperienceArc() {
...
/* ============================================================
   RENDER: CASE STUDIES ("What I've Built")
   ============================================================ */
...
/* ============================================================
   RENDER: HOW I THINK — 3 concrete decisions, 5-block structure
   (Problem / Signal-Evidence / Thinking / Decision / Outcome-Learning)
   Only fully-written decisions are shown — anything still carrying
   [CONTENT NEEDED] placeholders stays out of this view until filled in.
   ============================================================ */
function isDecisionComplete(d) {
  const fields = [d.problem, d.evidence, d.hypothesis, d.decision, d.result, d.learning];
  return fields.every((f) => typeof f === "string" && !f.startsWith("[CONTENT NEEDED"));
}
...
function renderDecisions(role) {
  ...
  const ready = PRODUCT_DECISIONS.filter(isDecisionComplete);
  const scored = [...ready].sort((a, b) => {
  ...
  scored.forEach((d, i) => {
    ...
    details.innerHTML = `
      <summary>
        ...
        <span class="decision-toggle">Problem → Outcome</span>
      </summary>
      <div class="decision-body">
        <div class="decision-row"><h5>Problem</h5><p>${d.problem}</p></div>
        <div class="decision-row"><h5>Signal / Evidence</h5><p>${d.evidence}</p></div>
        <div class="decision-row"><h5>Thinking</h5><p>${d.hypothesis}</p></div>
        <div class="decision-row"><h5>Decision</h5><p>${d.decision}</p></div>
        <div class="decision-row"><h5>Outcome / Learning</h5><p>${d.result}</p><p>${d.learning}</p></div>
      </div>
    `;
...
/* ============================================================
   RENDER: TOOLKIT (collapsed inside Experience by default)
   ============================================================ */
...
/* ============================================================
   RENDER: STATIC SECTIONS (beyond / hire me if / still learning / contact)
   ============================================================ */
...
/* ============================================================
   RENDER: COMPANY OVERLAY (growth-opportunity teardown)
   This is the ONLY block that reads COMPANY_PROFILES. Swapping
   companies means adding a key in data.js — nothing here changes.
   ============================================================ */
function renderCompanyOverlay(companySlug) {
  const profile = COMPANY_PROFILES[companySlug];
  ...
  $("#opportunities-title").textContent = `${profile.opportunities.length} growth opportunities I'd investigate at ${profile.name}.`;

  const grid = $("#opportunities-grid");
  grid.innerHTML = "";
  profile.opportunities.forEach((o, i) => {
    const card = el("div", "opportunity-card");
    card.innerHTML = `
      <p class="opportunity-num">0${i + 1}</p>
      <div class="decision-row"><h5>Observation</h5><p>${o.observation}</p></div>
      <div class="decision-row"><h5>Hypothesis</h5><p>${o.hypothesis}</p></div>
      <div class="decision-row"><h5>Experiment</h5><p>${o.experiment}</p></div>
      <div class="decision-row"><h5>Metric</h5><p>${o.metric}</p></div>
    `;
    grid.appendChild(card);
  });
}
...
/* ============================================================
   RENDER: SHOW YOUR WORK — reusable evidence gallery
   ============================================================ */
...
function renderEvidence() {
  ...
  EVIDENCE_ITEMS.forEach((item) => {
    ...
    const card = el(isAvailable ? "a" : "div", "evidence-card" + (isAvailable ? " available" : " placeholder"));
    if (isAvailable && item.anchor) card.href = item.anchor;
    card.innerHTML = `
      <span class="evidence-type">${EVIDENCE_TYPE_LABEL[item.type] || item.type}</span>
      <span class="evidence-label">${item.label}</span>
      <span class="evidence-source">${item.source}</span>
      <span class="evidence-status">${isAvailable ? "View →" : "Coming soon"}</span>
    `;
    grid.appendChild(card);
  });
}
...
/* ============================================================
   RENDER: HOW I BUILT THIS (the site's own product case study —
   User → JTBD once at the top, then Hypothesis → Decision →
   Tradeoff → Result per decision. Unlike How I Think, incomplete
   entries stay visible here — this section IS the working
   scaffold, shown honestly rather than hidden until finished.
   ============================================================ */
function renderWhyBuiltThis() {
  $("#why-built-headline").textContent = WHY_I_BUILT_THIS.headline;
  $("#why-built-intro").textContent = WHY_I_BUILT_THIS.intro;

  $("#built-context").innerHTML = `
    <div class="decision-row"><h5>User</h5><p>${WHY_I_BUILT_THIS.user}</p></div>
    <div class="decision-row"><h5>Job to be done</h5><p>${WHY_I_BUILT_THIS.jtbd}</p></div>
  `;

  const list = $("#built-decisions");
  list.innerHTML = "";
  WHY_I_BUILT_THIS.decisions.forEach((d) => {
    const rows = [d.hypothesis, d.decision, d.tradeoff, d.result];
    const isPlaceholder = rows.some((v) => typeof v === "string" && v.startsWith("[CONTENT NEEDED"));
    const card = el("div", "built-decision" + (isPlaceholder ? " placeholder" : ""));
    card.innerHTML = `
      <p class="built-decision-head">${d.title}</p>
      <div class="decision-body">
        <div class="decision-row"><h5>Hypothesis</h5><p>${d.hypothesis}</p></div>
        <div class="decision-row"><h5>Decision</h5><p>${d.decision}</p></div>
        <div class="decision-row"><h5>Tradeoff</h5><p>${d.tradeoff}</p></div>
        <div class="decision-row"><h5>Result / Learning</h5><p>${d.result}</p></div>
      </div>
    `;
...
/* ============================================================
   ...
   panels. Click to expand in place; nothing loads or shows until
   a visitor opts in.
   ============================================================ */
function initWhyAnnotations() {
  document.querySelectorAll("[data-why]").forEach((btn) => {
    const key = btn.getAttribute("data-why");
    const item = WHY_ANNOTATIONS[key];
    if (!item) { btn.hidden = true; return; }

    const panel = el("div", "why-panel", `<p>${item.body}</p>`);
    panel.hidden = true;
    ...
    btn.addEventListener("click", () => {
      ...
      if (opening) Analytics.track("why_annotation_opened", { key });
    });
  });
}
...
/* ============================================================
   START HERE — analytics only, native anchor links do the rest
   ============================================================ */
function initStartHere() {
  document.querySelectorAll('[data-track="start_here"]').forEach((a) => {
    a.addEventListener("click", () => Analytics.track("start_here_click", { id: a.getAttribute("data-id") }));
  });
}

/* ============================================================
   SIDE RAIL — active-section indicator + smooth-scroll nav
   ============================================================ */
function initRail() {
  ...
  const sections = links
    .map((link) => ({ link, section: document.getElementById(link.getAttribute("data-target")) }))
    .filter((x) => x.section);
...
function init() {
  ...
  renderStillLearning();
  renderBeyondResume();
  renderCompanyOverlay(currentCompany);
  renderEvidence();
  renderWhyBuiltThis();
  renderResumeModes();
  ...
  if (COMPANY_PROFILES[currentCompany]) setCompanyInUrl(currentCompany);
}
