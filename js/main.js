...
/* ============================================================
   RENDER: ROLE FOCUS STRIP (compact — replaces the old
   full-width "Explore Joe" section)
   ============================================================ */
function renderRoleGrid(activeRole) {
  ...
  Object.values(ROLE_PROFILES).forEach((profile) => {
    ...
    btn.innerHTML = `<span class="role-btn-label">${profile.label}</span>`;
...
function renderRoleSummary(role) {
  ...
  const tags = profile.emphasis.slice(0, 6).map((t) => `<span class="tag">${t}</span>`).join("");
  container.innerHTML = `
    <p class="role-summary-text">${profile.summary}</p>
...
/* ============================================================
   RENDER: COMPANY OVERLAY (banner + growth-opportunity teardown)
...
function renderCompanyOverlay(companySlug) {
  ...
  banner.hidden = false;
  banner.textContent = profile.tailorNote;
...
function renderWhyBuiltThis() {
  ...
  const grid = $("#why-built-grid");
  grid.innerHTML = "";
  WHY_I_BUILT_THIS.fields.forEach(({ label, value }) => {
    const isPlaceholder = typeof value === "string" && value.startsWith("[CONTENT NEEDED");
    const card = el("div", "personal-card" + (isPlaceholder ? " placeholder" : ""));
    card.innerHTML = `<h4>${label}</h4><p>${value}</p>`;
    grid.appendChild(card);
